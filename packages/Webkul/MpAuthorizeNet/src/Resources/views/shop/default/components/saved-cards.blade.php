...
@if(auth()->guard('customer')->check())
    <div class="mpauthorizenet-cards-block" id="saved-cards" style="display:none;">
        <div class="control-info mt-10 mb-10">
            @foreach($cards as $card)
                    <div class="authroizenet-card-info" id="{{ $card->id }}">
                        ...
                        <a id="delete-card" style="cursor: pointer;" data-id="{{ $card->id }}">{{ __('mpauthorizenet::app.delete') }}</a>
                    </div>
            @endforeach
        </div>
    </div>
