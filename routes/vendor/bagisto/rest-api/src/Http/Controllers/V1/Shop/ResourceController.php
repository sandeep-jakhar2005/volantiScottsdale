...
class ResourceController extends V1Controller implements ResourceContract
{
    ...
    {
        // sandeep || add relation order notes
     
        $resourceClassName = $this->resource();

        // $resource = $this->isAuthorized()
        //     ? $this->getRepositoryInstance()->with('orderNotes')->where('customer_id', $this->resolveShopUser($request)->id)->findOrFail($id)
        //     : $this->getRepositoryInstance()->with('orderNotes')->findOrFail($id);
